...
export default function MovieCreateModal({ open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    tmdbSearch(q)
      .then((r) => {
        if (!cancelled) setTmdbResults(Array.isArray(r) ? r : []);
      })
      .catch(() => {})
  ...
  const pickTmdb = (m: TMDBSearchItem) => {
    ...
    set("release_year", m.release_year || "");
    set("synopsis", m.overview || "");
    if (m.poster_url && !form.poster_url) {
      set("poster_url", m.poster_url);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Создать оффлайн фильм (B2B)</DialogTitle>
        </DialogHeader>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            <Label>Поиск по PoiskKino</Label>
            <Input placeholder="Введите название фильма для поиска..." value={tmdbQ} onChange={(e) => setTmdbQ(e.target.value)} />
                ...
                {tmdbResults.slice(0, 8).map((item) => (
                  <button key={item.id} className="w-full text-left hover:bg-white/5 rounded px-2 py-1" onClick={() => pickTmdb(item)}>
                    <div className="font-medium">
                      {item.title} <span className="text-gray-400">{item.release_year || ""}</span>
                    </div>
          ...
          <div>
            <Label>Локализованное название</Label>
          ...
          <div className="md:col-span-2">
            <Label>Краткое описание (синопсис)</Label>
          ...
          <div>
            ...
            <Input placeholder="например, 12+" value={form.age_rating} onChange={(e) => set("age_rating", e.target.value)} />
          </div>
          ...
          <div>
            <Label>Ожидаемые сборы (руб.)</Label>
          ...
          <div className="md:col-span-2">
            <Label>Подписанный URL (если нужен)</Label>
