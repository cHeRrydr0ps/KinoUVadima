...
```bash
# 1. Запуск всех сервисов с пересборкой
docker compose up --build

# 2. Запуск в фоне
docker compose up -d

# 3. Остановить и удалить все контейнеры
docker compose down

# 4. Остановить и удалить контейнеры + тома БД
docker compose down --volumes

# 5. Посмотреть статус контейнеров
docker compose ps

# 6. Логи конкретного сервиса
docker compose logs -f <service_name>

# 7. Перезапустить сервис
docker compose restart <service_name>

# 8. Запустить только один сервис
docker compose up <service_name>

# 9. Выполнить команду внутри контейнера
docker compose exec <service_name> <command>

# 10. Список образов Docker
docker images
```
